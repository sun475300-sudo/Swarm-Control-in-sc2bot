...
**작성일**: 2026-01-15  
**제거할 API 키**: `AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM`
...
`passwords.txt` 파일 생성:
```
AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM==>[API_KEY_REMOVED]
```
...
```bash
# API 키가 제거되었는지 확인
git log --all -S "AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM"
...
git filter-branch --force --index-filter \
    "git ls-files -z | xargs -0 sed -i 's/AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM/[API_KEY_REMOVED]/g'" \
...
.\tools\remove_api_key_from_git_history.ps1 -ApiKey "AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM"
```
...
./tools/remove_api_key_from_git_history.sh "AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM"
```
...
```bash
# 모든 커밋에서 검색
git log --all --source --full-history -S "AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM"

# 특정 파일에서 검색
git log --all --source --full-history -S "AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM" -- "wicked_zerg_challenger/NEW_API_KEY_SETUP.md"
```
...
```bash
# 현재 작업 디렉토리에서 검색
grep -r "AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM" .
```
