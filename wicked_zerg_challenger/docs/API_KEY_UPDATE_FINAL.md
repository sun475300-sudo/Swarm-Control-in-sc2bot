...
**작성일**: 2026-01-14  
**새 키**: `AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM`
...
### 1. 키 파일
- ? `secrets/gemini_api.txt` → `AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM`
- ? `api_keys/GEMINI_API_KEY.txt` → `AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM`
- ? `api_keys/GOOGLE_API_KEY.txt` → `AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM`

### 2. 환경 설정
- ? `.env` → `GEMINI_API_KEY=AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM`
- ? `.env` → `GOOGLE_API_KEY=AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM`
- ? 사용자 환경 변수 → 업데이트됨
- ? 현재 세션 환경 변수 → 업데이트됨

### 3. Android 설정
- ? `monitoring/mobile_app_android/local.properties` → `GEMINI_API_KEY=AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM`
...
**예상 출력**: `AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM`
...
**예상 결과**: `AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM`
...
1. **API 키 제한**
   ...
   - 키 `AIzaSyBDdPWJyXs56AxeCPmqZpySFOVPjjSt_CM` 선택
   - "API 제한" → "키 제한" 선택
